...
def generate_annotation2(ref_gff, types, outfld):
    """
    Filtra el GFF según los tipos especificados y descarta Apicoplast.
    Para cada protein_coding_gene, ajusta sus posiciones start/stop a las de la primera CDS.
    Guarda todo en un único fichero con sufijo '_filtered_only_genes.gff'.
    """
    # Cargar GFF completo
    gff = pb.BedTool(ref_gff)

    # Preparar mapeo transcript -> gen
    transcript_to_gene = {}
    for feat in gff:
        if feat.fields[2] in ('mRNA', 'transcript') and feat.chrom != 'Pf3D7_API_v3':
            attrs = dict(item.split('=') for item in feat.fields[8].split(';') if '=' in item)
            tid = attrs.get('ID')
            gid = attrs.get('Parent')
            if tid and gid:
                transcript_to_gene[tid] = gid.split(',')[0]

    # Agrupar CDS por gen
    gene_cds = defaultdict(list)
    for feat in gff.filter(lambda x: x.fields[2] == 'CDS' and x.chrom != 'Pf3D7_API_v3'):
        attrs = dict(item.split('=') for item in feat.fields[8].split(';') if '=' in item)
        for tid in attrs.get('Parent', '').split(','):
            gid = transcript_to_gene.get(tid)
            if gid:
                gene_cds[gid].append(feat)

    # Calcular primera CDS por hebra
    first_coords = {}
    for gid, feats in gene_cds.items():
        feats_sorted = sorted(feats, key=lambda x: x.start)
        strand = feats_sorted[0].strand
        cds_feat = feats_sorted[0] if strand == '+' else feats_sorted[-1]
        first_coords[gid] = (cds_feat.start, cds_feat.end)

    # Filtrar genes y otros tipos, descartar Apicoplast
    filtered = gff.filter(lambda x: x.fields[2] in types and x.chrom != 'Pf3D7_API_v3')

    # Modificar posiciones de genes codificantes
    final_feats = []
    for feat in filtered:
        ftype = feat.fields[2]
        if ftype == 'protein_coding_gene':
            attrs = dict(item.split('=') for item in feat.fields[8].split(';') if '=' in item)
            gid = attrs.get('ID')
            coords = first_coords.get(gid)
            if coords:
                # copiar y ajustar start/end
                new_feat = copy.copy(feat)
                new_feat.start, new_feat.end = coords
    ...
    # Guardar en único archivo
    p = Path(ref_gff)
    ext = get_extension(ref_gff)
    base = p.name.replace(ext, '')
    outfld = Path(outfld)
    outfld.mkdir(parents=True, exist_ok=True)
    outname = f"{base}_filtered_only_genes.gff"
    output = outfld / outname
    pb.BedTool(final_feats).saveas(str(output))
    sort_gff(str(output))
...
def cross_coverage_with_bed(cov_file, bed_file, outdir):
    ...
    cov = bed.sort().map(coverage, c = 4, o='mean') #c=4 means fourth parameter in coverage is score (rpkm,...)
...
def genomewise_coverage(input_file):
    ...
    if out_table:
        ...
        covs_x = [[x[3] for x in f] for f in parsed_outs]   # Join the cov values from all files
