...
###########################################################################################################################################3
OUT = "/PROJECTES/MALARIA_EPIGENETICS/Projects/ReCHIP_GDV1_Project_ene25/Proteomics/cho/NF54_cho_ProtResults.csv"
fold_change_data = pd.read_csv('/PROJECTES/MALARIA_EPIGENETICS/Projects/ReCHIP_GDV1_Project_ene25/Proteomics/cho/proteomics.csv')
treat = 'choDep'
#treat = 'DHA'
ps = 1000
...
#treatment = ['Ctl', 'Ctl', 'DHA', 'Ctl', 'DHA', 'DHA']
treatment = ['Ctl', 'Ctl', 'Ctl', 'choDep', 'choDep', 'choDep']
#treatment = ['Ctl', 'Ctl', 'Ctl', 'DHA', 'DHA', 'DHA']
experiment_number = [1,2,3,1,2,3]
...
data['fold_change'] = pd.to_numeric(data['fold_change'], errors='coerce') + ps
...
for gene in data['Gene'].unique():
    ...
    for exp in gene_data['experiment_number'].unique():
        ...
        if len(ctl_value) > 0 and len(choDep_value) > 0:
            lfc = math.log2(choDep_value[0]/ctl_value[0])  # Calcular LFC para cada experimento
    ...
    print(gene, ctl_value, choDep_value)
